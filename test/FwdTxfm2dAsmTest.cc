...
TEST(LbdFwdExtremeRange, dct_dct_bitexact) {
    ...
    struct {
        ...
        {TX_16X16, 16, svt_av1_transform_two_d_16x16_c, svt_av1_fwd_txfm2d_16x16_neon},
        {TX_32X32, 32, svt_av1_transform_two_d_32x32_c, svt_av1_fwd_txfm2d_32x32_neon},
    };
        ...
        for (int pat = 0; pat < 5; pat++) {
            for (int i = 0; i < c.n; i++)
                for (int j = 0; j < c.n; j++) {
                    ...
                    case 2: v = ((i + j) & 1) ? 255 : -255; break; // checkerboard
                    case 3: v = (j & 1) ? 255 : -255; break;       // vertical stripes
                    default: v = (i & 1) ? 255 : -255; break;      // horizontal stripes
                    }
                    in[i * stride + j] = v;
                }
