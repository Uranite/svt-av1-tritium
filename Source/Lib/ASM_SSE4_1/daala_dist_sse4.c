...
void svt_aom_od_compute_diff_and_filter_h_sse4_1(
    ...
    uint32_t width, uint32_t height)
{
    for (uint32_t i = 0; i < height; i++) {
        ...
        // Apply filter to middle elements using SSE
...
void svt_aom_od_filter_v_sse4_1(
    ...
    uint32_t width, uint32_t height)
{
    ...
    for (uint32_t i = 1; i < height - 1; i++) {
        ...
        for (uint32_t j = 0; j < width; j += 4) {
            __m128i prev = _mm_loadu_si128((const __m128i*)&row_prev[j]);
            __m128i curr = _mm_loadu_si128((const __m128i*)&row_curr[j]);
            __m128i next = _mm_loadu_si128((const __m128i*)&row_next[j]);

            __m128i curr_5 = _mm_add_epi32(_mm_slli_epi32(curr, 2), curr);
            __m128i res = _mm_add_epi32(_mm_add_epi32(curr_5, prev), next);
            _mm_storeu_si128((__m128i*)&row_lp[j], res);
        }
    }
...
static inline int od_compute_var_4x4_sse4_1(const uint16_t *x, uint32_t stride) {
...
void svt_aom_od_compute_var_and_dist_8x8_sse4_1(
    const uint16_t *x, uint32_t x_stride,
    const uint16_t *y, uint32_t y_stride,
    ...
    uint64_t *sum_e_lp_sq)
{
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            varx[idx] = od_compute_var_4x4_sse4_1(x + 2 * i * x_stride + 2 * j, x_stride);
            vary[idx] = od_compute_var_4x4_sse4_1(y + 2 * i * y_stride + 2 * j, y_stride);
            idx++;
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        __m128i lo = _mm_loadu_si128((const __m128i*)&row[0]);
        __m128i hi = _mm_loadu_si128((const __m128i*)&row[4]);

        __m128i sq_lo = _mm_mullo_epi32(lo, lo);
        __m128i sq_hi = _mm_mullo_epi32(hi, hi);

        __m128i sq_lo_01 = _mm_cvtepi32_epi64(sq_lo);
        __m128i sq_lo_23 = _mm_cvtepi32_epi64(_mm_srli_si128(sq_lo, 8));
        __m128i sq_hi_01 = _mm_cvtepi32_epi64(sq_hi);
        __m128i sq_hi_23 = _mm_cvtepi32_epi64(_mm_srli_si128(sq_hi, 8));

        accum = _mm_add_epi64(accum, sq_lo_01);
        accum = _mm_add_epi64(accum, sq_lo_23);
        accum = _mm_add_epi64(accum, sq_hi_01);
        accum = _mm_add_epi64(accum, sq_hi_23);
    }

    DECLARE_ALIGNED(16, uint64_t, sum_val[2]);
    _mm_storeu_si128((__m128i*)sum_val, accum);
    *sum_e_lp_sq = sum_val[0] + sum_val[1];
}
