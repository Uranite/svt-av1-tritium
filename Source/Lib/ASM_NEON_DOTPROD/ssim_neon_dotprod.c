...
// Low-bit-depth SSIM accumulation using the Armv8.2 dot-product (UDOT) extension:
// each UDOT collapses a sum-of-products of four bytes into one accumulator lane, so
// the per-byte squares (sum_sq) and cross-products (sum_sxr), and the byte sums (via
// a vector of ones), each cost a single instruction per 16 samples. The integer sums
// are bit-identical to svt_ssim_*_c, so the returned double matches exactly.
...
double svt_ssim_4x4_neon_dotprod(const uint8_t* s, uint32_t sp, const uint8_t* r, uint32_t rp) {
    ...
    const uint8x16_t ones = vdupq_n_u8(1);
    return similarity(vaddvq_u32(vdotq_u32(vdupq_n_u32(0), vs, ones)),
